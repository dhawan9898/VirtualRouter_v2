...
GLTHREAD_TO_STRUCT(glue_to_pkt_q, pkt_q_t, glue);
...
#define EV_DIS_LOCK(ev_dis_ptr)   (pthread_mutex_lock(&((ev_dis_ptr)->ev_dis_mutex)))

#define EV_DIS_UNLOCK(ev_dis_ptr) (pthread_mutex_unlock(&((ev_dis_ptr)->ev_dis_mutex)))

/* To be used by applications */
task_t *event_dispatcher_get_current_task();

void event_dispatcher_init();

void event_dispatcher_run();
...
void pkt_q_enqueue(pkt_q_t *pkt_q, char *pkt, uint32_t pkt_size);
