...
static void event_dispatcher_schedule_task(task_t *task){
	...
	if(debug) 
        printf("Task Added to Dispatcher's Queue\n");
	...
	if (ev_dis.ev_dis_state == EV_DIS_IDLE &&
		...
		if(debug) 
            printf("signal sent to dispatcher\n");
	...
	if (task->app_cond_var) {
		...
		pthread_cond_wait(task->app_cond_var, &ev_dis.ev_dis_mutex);
...
static void event_dispatcher_process_task_post_call(task_t *task){
	...
	switch(task->task_type) {
		...
		case TASK_PKT_Q_JOB:	
			...
			if (IS_GLTHREAD_LIST_EMPTY(&pkt_q->q_head)) {
				if(debug) 
                    printf("Queue Exhausted, will stop untill pkt enqueue..\n");
				pthread_mutex_unlock(&pkt_q->q_mutex);
				return;
			}
			pthread_mutex_unlock(&pkt_q->q_mutex);
			if(debug) 
                printf("more pkts in Queue, will continue..\n");
			event_dispatcher_schedule_task(task);
			break;
...
static task_t *event_dispatcher_get_next_task_to_run(){
	...
	while(1) {
		...
		if(!task) {
			...
			if(ev_dis.ev_dis_state != EV_DIS_TASK_FIN_WAIT){
				...
				if(debug) 
                    printf("EVE DIS moved to EV_DIS_TASK_FIN_WAIT, dispatching the task\n");
			}
			...
			event_dispatcher_process_task_post_call(task);
...
static task_t *create_new_task(void *arg, uint32_t arg_size, event_cbk cbk){
...
void task_schedule_again(task_t *task){

	if(task == NULL) {
		task = event_dispatcher_get_current_task();
	}
	assert(task->task_type == TASK_ONE_SHOT);
	task->re_schedule = true;
}

void event_dispatcher_run(void){
	...
	pthread_create(event_dis_thread, &attr, event_dispatcher_thread, NULL);
}

task_t *event_dispatcher_get_current_task(void){

	return ev_dis.current_task;
}


task_t *task_create_new_job(void *data, event_cbk cbk, task_type_t task_type) {
...
task_t *task_create_new_job_synchronous(void *data, event_cbk cbk, task_type_t task_type) {
...
void task_cancel_job(task_t *task){
...
static pkt_t *task_get_new_pkt(char *pkt, uint32_t pkt_size){
...
char *task_get_next_pkt(uint32_t *pkt_size){
...
void pkt_q_enqueue(pkt_q_t *pkt_q, char *_pkt, uint32_t pkt_size){
	...
	EV_DIS_UNLOCK(&ev_dis);
	if (debug) 
        printf("%s() calling event_dispatcher_schedule_task()\n", __FUNCTION__);
	event_dispatcher_schedule_task(pkt_q->task);	
}

void init_pkt_q(pkt_q_t *pkt_q, event_cbk cbk){

	init_glthread(&pkt_q->q_head);
	pthread_mutex_init(&pkt_q->q_mutex, NULL);
	pkt_q->task = create_new_task((void *)pkt_q, sizeof(*pkt_q), cbk);
