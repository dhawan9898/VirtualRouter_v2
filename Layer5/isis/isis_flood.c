...
static void isis_lsp_xmit_job(void *arg, uint32_t arg_size)
{
    glthread_t *curr;
...
void isis_start_lsp_pkt_periodic_flooding(node_t *node)
{
    ...
    if(!node_info)
        return;
    if(node_info->periodic_lsp_flood_timer)
        return;
    node_info->periodic_lsp_flood_timer = timer_register_app_event(node_get_timer_instance(node), isis_timer_wrapper_lsp_flood,\
                                                                    (void *)node, sizeof(node_t), ISIS_LSP_DEFAULT_FLOOD_INTERVAL, 1);
}
...
void isis_queue_lsp_pkt_for_transmission(interface_t *intf, isis_lsp_pkt_t *lsp_pkt)
{
    ...
    if(!isis_node_intf_is_enable(intf))
        return;
    intf_info = ISIS_INTF_INFO(intf);
    if(!(intf_info->adjacency && intf_info->adjacency->adj_state == ISIS_ADJ_STATE_UP))
        return;
...
void isis_flood_lsp_synchronously(node_t *node, isis_lsp_pkt_t *lsp_pkt)
{
    interface_t *intf;
    isis_intf_info_t *intf_info;
...
void isis_create_and_flood_purge_lsp_pkt_synchronously(node_t *node)
{
    isis_node_info_t *node_info = ISIS_NODE_INFO(node);
    if(!node_info)
        return;
